/* $Id: main.cc 872 2008-08-07 15:10:37Z nuno-lopes $ */
#ifdef HAVE_CONFIG_H
#include "config.h"
#elif defined(_WIN32)
...
static void usage()
{
	std::cerr << "usage: re2j [-bcdDefFghirsuvVw1] [-o of] [-t th] file\n"
	"\n"
...
int main(int argc, char *argv[])
{
	...
	if ((bFlag || fFlag) && bSinglePass) {
		std::cerr << "re2j: error: Cannot combine -1 and -b or -f switch\n";
		return 1;
	}
	if (!cFlag && headerFileName)
	{
		std::cerr << "re2j: error: Can only output a header file when using -c switch\n";
		return 2;
	}

	if (rFlag && eFlag)
	{
		std::cerr << "re2j: error: Cannot combine -e with -r switch\n";
		return 2;
	}
	if (wFlag && eFlag)
	{
		std::cerr << "re2j: error: Cannot combine -e with -w or -u switch\n";
		return 2;
	}
	if (wFlag && uFlag)
	{
		std::cerr << "re2j: error: Cannot combine -u with -w switch\n";
		return 2;
	}

	if (DFlag && (bFlag || dFlag || sFlag))
	{
		std::cerr << "re2j: error: Cannot combine -D with -b, -d  or -s switches\n";
		return 2;
	}
	...
	re2j::ifstream_lc source;

	if (sourceFileName[0] == '-' && sourceFileName[1] == '\0')
	{
		if (fFlag)
		{
			std::cerr << "re2j: error: multiple /*!re2j stdin is not acceptable when -f is specified\n";
			return 1;
		}
	...
	{
		cerr << "re2j: error: cannot open " << sourceFileName << "\n";
		return 1;
	}

	// set up the output stream
	re2j::ofstream_lc output;
	re2j::ofstream_lc header;
	...
	{
		cerr << "re2j: error: cannot open " << outputFileName << "\n";
		return 1;
	}
	if (headerFileName)
	{
		...
		{
			cerr << "re2j: error: cannot open " << headerFileName << "\n";
			return 1;
		}
	...
	if (!bSinglePass)
	{
		...
		re2j::ifstream_lc null_source;
		
		if (!null_source.open(sourceFileName).is_open())
		{
			cerr << "re2j: error: cannot re-open " << sourceFileName << "\n";
			return 1;
		}

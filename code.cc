/* $Id: code.cc 897 2009-01-08 16:44:25Z nuno-lopes $ */
#include <stdarg.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
...
static void genGoTo(std::ostream &o, uint ind, const State *from, const State *to, bool & readCh)
{
	...
	{
		o << indent(ind) << mapCodeName["yych"] << " = " << yychConversion << mapCodeName["YYDATA"] << "[" << mapCodeName["YYCURSOR"] << "];\n";
		readCh = false;
	}
...
static void genIf(std::ostream &o, uint ind, const char *cmp, uint v, bool &readCh)
{
	...
	if (readCh)
	{
		o << "(" << mapCodeName["yych"] << " = " << yychConversion << mapCodeName["YYDATA"] << "[" << mapCodeName["YYCURSOR"] << "])";
		readCh = false;
	}
	else
	{
...
static void need(std::ostream &o, uint ind, uint n, bool & readCh, bool bSetMarker)
{
	...
	if (n > 0)
	{
		if (bSetMarker)
		{
			o << indent(ind) << mapCodeName["yych"] << " = " << yychConversion << mapCodeName["YYDATA"] << "[(" << mapCodeName["YYMARKER"] << " = " << mapCodeName["YYCURSOR"] << ")];\n";
		}
		else
		{
			o << indent(ind) << mapCodeName["yych"] << " = " << yychConversion << mapCodeName["YYDATA"] << "[" << mapCodeName["YYCURSOR"] << "];\n";
...
void Enter::emit(std::ostream &o, uint ind, bool &readCh, const std::string&) const
{
	if (state->link)
	{
		...
		{
			o << indent(ind-1) << "case " << labelPrefix << label << ":\n";
		}
		need(o, ind, state->depth, readCh, false);
	}
	else
...
void Initial::emit(std::ostream &o, uint ind, bool &readCh, const std::string&) const
{
	...
	{
		o << indent(ind-1) << "case " << startLabelName << ":\n";
	}
...
void Go::genSwitch(std::ostream &o, uint ind, const State *from, const State *next, bool &readCh, uint mask) const
{
	...
	else
	{
		...
		if (!DFlag)
		{
			...
			if (readCh)
			{
				o << indent(ind) << "switch ((" << mapCodeName["yych"] << " = " << yychConversion << mapCodeName["YYDATA"] << "[" << mapCodeName["YYCURSOR"] << "])) {\n";
				readCh = false;
			}
			else
			{
...
void Go::genCpGoto(std::ostream &o, uint ind, const State *from, const State *next, bool &readCh) const
{
	...
	if (readCh)
	{
		sYych = "(" + mapCodeName["yych"] + " = " + yychConversion + mapCodeName["YYDATA"] + "[" + mapCodeName["YYCURSOR"] + "])";
	}
	else
	{
...
void DFA::emit(std::ostream &o, uint& ind, const RegExpMap* specMap, const std::string& condName, bool isLastCond, bool& bPrologBrace)
{
	...
	// NOTE: prolog_label must be yy0 because of the !getstate:re2j handling
	// in scanner.re
	...
	if (bProlog)
	{
		...
		if (cFlag && !DFlag)
		{
			...
			{
				o << indent(ind-1) << "case " << labelPrefix << prolog_label << ":\n";
			}
			if (!startLabelName.empty())
			{
				o << indent(ind-1) << "case " << startLabelName << ":\n";
...
{
	o << "/* Generated by re2j " PACKAGE_VERSION;
...
void Scanner::config(const Str& cfg, const Str& val)
{
	if (mapDefineKeys.empty())
	{
		...
		mapDefineKeys.insert("define:YYDATA");

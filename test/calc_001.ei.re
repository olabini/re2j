...
- basic interface for string reading
  ...
  . To make the output more readable we use 're2j:indent:top' scanner 
    configuration that configures re2j to prepend a single tab (the default)
  ...
  . The last expression accepts any input character. It tells re2j to accept 
    the opposit of the empty range. This includes numbers and our tokens but
    as re2j goes from top to botton when evaluating the expressions this is no 
    problem.
  . The first three rules show that re2j actually prioritizes the expressions 
    ...
    "0" [0-9]+
    "0" | ( [1-9] [0-9]* )
    A full description of re2j rule syntax can be found in the manual.
*/

/* $Id: scanner.re 894 2009-01-02 18:56:36Z helly $ */
#include <stdlib.h>
#include <string.h>
#include <iostream>
#include <sstream>
...
/*!re2c
	"/*!re2j"	{
					if (rFlag)
					{
						fatal("found standard 're2j' block while using -r flag");
					}
	...
	"/*!rules:re2j"	{
					...
					{
						fatal("found 'rules:re2j' block without -r flag");
					}
					if (bUsedYYMaxFill && bSinglePass)
					{
						fatal("found scanner block after YYMAXFILL declaration");
	...
	"/*!use:re2j"	{
					if (!rFlag)
					{
						fatal("found 'use:re2j' block without -r flag");
					}
					reuse();
					if (bUsedYYMaxFill && bSinglePass)
					{

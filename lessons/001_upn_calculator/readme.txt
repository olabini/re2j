re2j lesson 001_upn_calculator, (c) M. Boerger 2006

This lesson gets you started with re2j. In the end you will have an easy RPN
(reverse polish notation) calculator for use at command line.

You will learn about the basic interface of re2j when scanning input strings. 
How to detect the end of the input and use that to stop scanning in order to
avoid problems.

Once you have successfully installed re2j you can use it to generate *.c files
...
of re2j. This tells re2j to generate code that uses if statements rather 
...
from main.re. They assume that you unpacked the zip package and have re2j
itself build or installed in Release and Release-2005 directory respectively.
If re2j cannot be found you need to modify the custom build step and correct
the path to re2j.

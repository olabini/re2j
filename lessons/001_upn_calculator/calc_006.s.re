/* re2j lesson 001_upn_calculator, calc_006, (c) M. Boerger 2006 - 2007 */
/*!ignore:re2j

- avoiding YYFILL()
  . We use the inplace configuration re2j:yyfill to suppress generation of
    YYFILL() blocks. This of course means we no longer have to provide the
    macro.
  . We also drop the YYMARKER stuff since we know that re2j does not generate 
    it for this example.
  . Since re2j does no longer check for out of data situations we must do this.
    ...
    However with other rule sets re2j might read more then one character in a 
    row. In those cases it is normally hard to impossible to avoid YYFILL.

- optimizing the generated code by using -s command line switch of re2j
  . This tells re2j to generate code that uses if statements rather 

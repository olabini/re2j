/* re2j lesson 001_upn_calculator, calc_008, (c) M. Boerger 2006 - 2007 */
/*!ignore:re2j

- using -b with signed character input
  . Since the code is being generated with -b switch re2j requires the internal
    ...
    function. Other re2j generated code often have the scanners work completely
    on unsigned input. Thus requesting a conversion.
    
    To avoid the conversion on input, re2j allows to do the conversion when
    reading the internal yych variable. To enable that conversion you need to
    use the implace configuration 're2j:yych:conversion' and set it to 1. This
...
- More inplace configurations for better/nicer code
  . re2j allows to overwrite the generation of any define, label or variable
    used in the generated code. For example we overwrite the 'yych' variable
    name to 'curr' using inplace configuration 're2j:variable:yych = curr;'.
  ...
  . We further more use inplace configurations instead of defines. This allows
    ...
    to 'YYCTYPE' when placing 're2j:define:YYCTYPE = "unsigned char";' infront 
    of 're2j:yych:conversion'. Note that we have to use apostrophies for the
    first setting as it contains a space.

  . Last but not least we use 're2j:labelprefix = scan' to change the prefix
    of generated labels.
*/
...
/*!re2j
	...
	re2j:variable:yych   = curr;
	re2j:indent:top      = 2;
	re2j:yyfill:enable   = 0;
	re2j:yych:conversion = 1;
	re2j:labelprefix     = scan;
